   ...
   CREATE OR REPLACE TABLE "CHANDU2"."MATERIAL_USER_ID_STITCHER_ED62A9A5_16_INTERNAL_MAPPING" AS (
    SELECT
        node_id, node_id_type, user_main_id, valid_at, first_seen_at, user_main_id_dist, stitching_active
    FROM "MATERIAL_USER_ID_STITCHER_ED62A9A5_16_INTERNAL_FINISHED_MAPPING"
    UNION ALL
    SELECT
        node_id, node_id_type, user_main_id, valid_at, first_seen_at, user_main_id_dist, stitching_active
...
/* Update the main_id for a cluster to be the one generated by the earliest id seen in that cluster */
 ...
 AS generated_user_main_id,
            ...
            node_id, node_id_type, user_main_id, valid_at, first_seen_at, user_main_id_dist, stitching_active
        FROM
            "CHANDU2"."MATERIAL_USER_ID_STITCHER_ED62A9A5_16_INTERNAL_MAPPING"
        )
);  


   CREATE OR REPLACE VIEW "MATERIAL_USER_ID_STITCHER_ED62A9A5_16" AS (
    WITH ranked_mappings AS (
        ...
        FROM  "CHANDU2"."MATERIAL_USER_ID_STITCHER_ED62A9A5_16_INTERNAL_MAPPING"
    )
    SELECT
        user_main_id,
        other_id,
   ...
   DROP TABLE IF EXISTS "MATERIAL_USER_ID_STITCHER_ED62A9A5_16_INTERNAL_ACTIVE_MAPPING";  


   DROP TABLE IF EXISTS "MATERIAL_USER_ID_STITCHER_ED62A9A5_16_INTERNAL_FINISHED_MAPPING";  
